 ...
 * FileName:  OnlineRecommender.scala
 * Author:  GaryMK
 * Date:  2021/1/5 下午1:40
 * Version:  1.0
 * LastModified:  2021/1/5 下午1:40
 *
 */

package com.morningstar.online
...
object OnlineRecommender {
    ...
    ratingStream.foreachRDD{
      rdds => rdds.foreach{
        case (userId, productId, score, timestamp) =>
          ...
          val userRecentlyRatings = getUserRecentlyRating(MAX_USER_RATING_NUM, userId, ConnHelper.jedis)
          ...
          // 3.计算每个被选商品的推荐优先级，得到当前用户的实时推荐列表,保存成Array[(productId, score)]
  ...
  def getUserRecentlyRating(num: Int, userId: Int, jedis: Jedis): Array[(Int, Double)] = {
    // 从redis中用户的评分队列里获取评分数据，list键名为uid:USERID,值格式为PRODUCTID:SCORE
